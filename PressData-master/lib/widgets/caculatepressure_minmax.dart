...
PressureStats calculatePressureDemoStats(
    List<PressDataDemoTableData> data, List<String> selectedValues) {
  if (data.isEmpty) {
    return PressureStats(
      maxPressure: [],
      maxPressureTime: [],
  ...
  for (String value in selectedValues) {
    int maxPressure = data.first.getPressureDemoValue(value);
    DateTime maxPressureTime = data.first.recordedAt!;
    int minPressure = data.first.getPressureDemoValue(value);
    DateTime minPressureTime = data.first.recordedAt!;
    int totalPressure = 0;

    for (var record in data) {
      int currentPressure = record.getPressureDemoValue(value);
      if (currentPressure > maxPressure) {
        maxPressure = currentPressure;
        maxPressureTime = record.recordedAt!;
      }
...
extension on PressDataDemoTableData {
  int getPressureDemoValue(String value) {
    switch (value) {
      case 'O2(1)':
        return o2;
      case 'TEMP':
