...
class WeeklyChartLive extends StatefulWidget {
  DateTime stardate;
  DateTime endDate;
  ...
  @override
  State<WeeklyChartLive> createState() => _WeeklyChartLiveState();
}

class _WeeklyChartLiveState extends State<WeeklyChartLive> {
  @override
...
class _LineChartScreenState extends State<LineChartScreen> {
  ...
  void generatePDF_weekly() async {
    ...
    _selectedWeeklyDateRange =
        DateTimeRange(start: widget.stardate, end: widget.endDate);
        ...
        child: pw.Column(
          ...
          children: [
            ...
            pw.Row(
              ...
              children: [
                pw.Text(
                    'Date: ${DateFormat('dd-MM-yyyy').format(_selectedWeeklyDateRange!.start)} to ${DateFormat('dd-MM-yyyy').format(_selectedWeeklyDateRange!.end)}',
                    style: regularStyle),
                pw.Text(
  ...
  Future<void> fetchData(DateTime date) async {
    List<PressDataTableData> data = await db!
        .getDataForDateRange(widget.stardate, widget.endDate, widget.serial);
    print("Above Errorcall");
    List<ErrorTableData> error_data = await db!
        .getErrorDataForWeek(widget.stardate, widget.endDate, widget.serial);
    print("List of Values->>>>$error_data");
    if (data.isEmpty) {
      setState(() {
  ...
  Future<void> _captureAndShowImage(BuildContext context) async {
    ...
    try {
      ...
      Navigator.of(context).pop();
      generatePDF_weekly(); // Pop DailyChart page
    } catch (e) {
