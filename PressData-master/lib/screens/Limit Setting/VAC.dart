...
class VAC extends StatefulWidget {
  final int minvac;
  final int maxvac;
  const VAC({super.key, required this.maxvac, required this.minvac});
...
class _VACState extends State<VAC> {
  int maxLimit = 175;
  int minLimit = 100;
  late int max = widget.maxvac;
  late int min = widget.minvac;
  ...
  void updateMaxLimit(double value) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final newvalue = (value.clamp(min.toDouble() + 1, 750)).toInt();
    setState(() {
      max = newvalue;
  ...
  void updateMinLimit(double value) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final newvalue = (value.clamp(50.0, max.toDouble() - 1.0)).toInt();
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    _updateData('VAC_DATA', min.toDouble(), max.toDouble());
